...
var numberOfItems = $('#campusnewspagination .news').length; // Get total number of the items that should be paginated
var limitPerPage = 8; // Limit of items per each page
$('#campusnewspagination .news:gt(' + (limitPerPage - 1) + ')').hide(); // Hide all items over page limits (e.g., 16th item, 17th item, etc.)
...
$(".pagination").append("<li class='page-item' id='first-page'><a class='page-link' href='#' aria-label='First'>" + "First Page" + "</a></li>");

//Add last page button
$(".pagination").append("<li class='page-item' id='last-page'><a class='page-link' href='#' aria-label='Last'>" + "Last Page" + "</a></li>");
...
$(".pagination li.current-page").on("click", function() {
  ...
  if ($(this).hasClass('active')) {
    ...
    $("#campusnewspagination .news").hide(); // Hide all items in loop, this case, all the list groups
    var grandTotal = limitPerPage * currentPage; // Get the total number of items up to the page number that was clicked on

    // Loop through total items, selecting a new set of items based on page number
    for (var i = grandTotal - limitPerPage; i < grandTotal; i++) {
      $("#campusnewspagination .news:eq(" + i + ")").show(); // Show items from the new page that was selected
    }
  }
...
$("#next-page").on("click", function() {
  ...
  if (currentPage === totalPages) {
    ...
    $("#campusnewspagination .news").hide(); // Hide all items in the pagination loop
...
// Function to navigate to the last page when users click on the last-page id (last page button)
$("#last-page").on("click",function(){
  ...
  if (currentPage === totalPages) {
    ...
    $(".pagination li").removeClass('active'); // Remove the 'activate' status class from the previous active page number
    $("#campusnewspagination .news").hide();
    var grandTotal = limitPerPage * totalPages;// Get the total number of items up to the page before the last page


    for( var i = grandTotal-limitPerPage; i<= grandTotal; i++){
      $("#campusnewspagination .news:eq(" + i + ")").show(); // Show items from the last page
    }
    $(".pagination li.current-page:eq(" + (currentPage - 1) + ")").addClass('active'); // Make new page number the 'active' page
    $("#paginationlabel").html("<p class='text-muted font-italic mt-2' style='font-size:0.8rem;'>Page " + totalPages + " out of " + totalPages + "</p>");// Change the current page indicator when the previous button is pressed
...
$("#first-page").on("click",function(){
  ...
  if (currentPage === 1) {
    ...
    $(".pagination li").removeClass('active'); // Remove the 'activate' status class from the previous active page number
    $("#campusnewspagination .news").hide();
    var grandTotal = limitPerPage * currentPage;// Get the total number of items up to the page before the last page
